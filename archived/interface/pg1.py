...
date_time = traffic_incidents['Message'].str.split(
    " ", n=1, expand=True).iloc[:, 0]
message = traffic_incidents['Message'].str.split(
    " ", n=1, expand=True).iloc[:, 1]
...
d_table_in = dash_table.DataTable(
    ...
    columns=[{"name": ["Traffic Incidents", i], "id": i}
             for i in traffic_incidents_new.columns[-3:]],
...
layout = html.Div(children=[
    ...
    # filter box
    html.Div(children=[
        html.H6("Select Road to view traffic images",
                style={'font-weight': 'bold'}),
        dcc.Dropdown(id="exp_dd",
                     options=[
                         {'label': 'All' + " (" + str(len(list(filter(
                             lambda x: "jpg" in x, os.listdir(image_folder))))) + ")", 'value': 'All'},
                         {'label': 'KPE' + \
                             " (" + str(len(d_exp_cam['KPE'])) + ")", 'value': 'KPE'},
                         {'label': 'ECP' + \
                             " (" + str(len(d_exp_cam['ECP'])) + ")", 'value': 'ECP'},
                         {'label': 'KJE' + \
                             " (" + str(len(d_exp_cam['KJE'])) + ")", 'value': 'KJE'},
                         {'label': 'SLE' + \
                             " (" + str(len(d_exp_cam['SLE'])) + ")", 'value': 'SLE'},
                         {'label': 'MCE' + \
                             " (" + str(len(d_exp_cam['MCE'])) + ")", 'value': 'MCE'},
                         {'label': 'CTE' + \
                             " (" + str(len(d_exp_cam['CTE'])) + ")", 'value': 'CTE'},
                         {'label': 'TPE' + \
                             " (" + str(len(d_exp_cam['TPE'])) + ")", 'value': 'TPE'},
                         {'label': 'BKE' + \
                             " (" + str(len(d_exp_cam['BKE'])) + ")", 'value': 'BKE'},
                         {'label': 'PIE' + \
                             " (" + str(len(d_exp_cam['PIE'])) + ")", 'value': 'PIE'},
                         {'label': 'AYE' + \
                             " (" + str(len(d_exp_cam['AYE'])) + ")", 'value': 'AYE'},
                         {'label': 'Woodlands Causeway/Johor' + " (" + str(len(
                             d_exp_cam['Woodlands Causeway/Johor'])) + ")", 'value': 'Woodlands Causeway/Johor'},
                         {'label': 'Tuas/Johor' + \
                             " (" + str(len(d_exp_cam['Tuas/Johor'])) + ")", 'value': 'Tuas/Johor'},
                         {'label': 'Changi' + \
                             " (" + str(len(d_exp_cam['Changi'])) + ")", 'value': 'Changi'},
                         {'label': 'City' + \
                             " (" + str(len(d_exp_cam['City'])) + ")", 'value': 'City'},
                         {'label': 'Moulmein' + \
                             " (" + str(len(d_exp_cam['Moulmein'])) + ")", 'value': 'Moulmein'},
                         {'label': 'Yio Chu Kang' + \
                             " (" + str(len(d_exp_cam['Yio Chu Kang'])) + ")", 'value': 'Yio Chu Kang'},
                         {'label': 'Jalan Bukit Merah' + " (" + str(
                             len(d_exp_cam['Jalan Bukit Merah'])) + ")", 'value': 'Jalan Bukit Merah'},
                         {'label': 'Jurong' + \
                             " (" + str(len(d_exp_cam['Jurong'])) + ")", 'value': 'Jurong'},
                         {'label': 'Airport' + \
                             " (" + str(len(d_exp_cam['Airport'])) + ")", 'value': 'Airport'},
                         {'label': 'Xilin Ave' + \
                             " (" + str(len(d_exp_cam['Xilin Ave'])) + ")", 'value': 'Xilin Ave'},
                         {'label': 'Marine Parade' + \
                             " (" + str(len(d_exp_cam['Marine Parade'])) + ")", 'value': 'Marine Parade'},
                         {'label': 'Telok Blangah' + \
                             " (" + str(len(d_exp_cam['Telok Blangah'])) + ")", 'value': 'Telok Blangah'},
                         {'label': 'Sentosa' + \
                             " (" + str(len(d_exp_cam['Sentosa'])) + ")", 'value': 'Sentosa'},
                         {'label': 'Toa Payoh' + \
                             " (" + str(len(d_exp_cam['Toa Payoh'])) + ")", 'value': 'Toa Payoh'},
                         {'label': 'Thomson' + \
                             " (" + str(len(d_exp_cam['Thomson'])) + ")", 'value': 'Thomson'},
                         {'label': 'Pasir Ris Dr 12' + " (" + str(
                             len(d_exp_cam['Pasir Ris Dr 12'])) + ")", 'value': 'Pasir Ris Dr 12'},
                         {'label': 'Punggol/Sengkang' + " (" + str(
                             len(d_exp_cam['Punggol/Sengkang'])) + ")", 'value': 'Punggol/Sengkang'},
                         {'label': 'Choa Chu Kang' + \
                             " (" + str(len(d_exp_cam['Choa Chu Kang'])) + ")", 'value': 'Choa Chu Kang'},
